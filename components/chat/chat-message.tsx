...
export function ChatMessage({ message, onRetry }: { message: ChatMessageType; onRetry?: () => void }) {
  ...
  if (message.status === "thinking") return (
    <article className="chat-message ai-message"><span className="message-avatar"><IdeonMark className="message-avatar-mark" /></span><div><strong>Ideon</strong><ThinkingIndicator /></div></article>
  );
