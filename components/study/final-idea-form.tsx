...
export function FinalIdeaForm() {
  ...
  const update = (field: keyof FinalIdea, value: string) => { setTouched(true); setSaveState("saving"); setIdea((current) => ({ ...current, [field]: value })); };

  useEffect(() => {
    const session = readClientSession();
    if (!session) { router.replace("/study/consent"); return; }
    fetch("/api/study/final", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "load", sessionId: session.sessionId, sessionToken: session.sessionToken }) })
      .then((response) => response.ok ? response.json() as Promise<{ idea?: FinalIdea | null }> : Promise.reject())
      .then((data) => { if (data.idea) setIdea(data.idea); })
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      fetch("/api/study/final", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "save", sessionId: session.sessionId, sessionToken: session.sessionToken, idea }) })
        .then((response) => { if (!response.ok) throw new Error(); setSaveState("saved"); })
        .catch(() => setSaveState("error"));
    }, 700);
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    const session = readClientSession();
    if (!complete || !session || submitting) return;
    setSubmitting(true); setSaveState("saving");
    try {
      const response = await fetch("/api/study/final", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "submit", sessionId: session.sessionId, sessionToken: session.sessionToken, idea }) });
  ...
  return (
    <section className="form-panel final-panel" aria-labelledby="final-title">
      ...
      <form onSubmit={submit}>
        ...
        <div className="unsaved-note" role="status"><i className={saveState === "saving" ? "changed" : ""} />{saveState === "saving" ? "Saving draft..." : saveState === "saved" ? "Draft saved securely." : saveState === "error" ? "Could not save. Check your connection." : "Your draft will save automatically."}</div>
        <div className="split-actions"><Button asChild variant="ghost"><Link href="/study/chat"><ArrowLeft /> Return to Chat</Link></Button><Button type="submit" disabled={!complete || submitting} className="action-button">{submitting ? "Submitting..." : "Submit Final Idea"} <ArrowRight /></Button></div>
      </form>
    </section>
  );
}
